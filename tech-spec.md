...
| **Summon bar** | Free-text type-to-conjure (Scribblenauts-style): `T` opens the bar, typing a noun + Enter spawns the object near the player, `Esc` closes. Medieval words only — modern ones get "It's 1178 — what's a ___?" and cost nothing. Ink-limited (50 per run in beat 1) with no live cap — `X` banishes the nearest conjured thing for a 1-ink refund (tune ink down per-level for difficulty). Curated lexicon (~40 everyday objects: ladder, bridge, plank, stone…) plus a procedural fallback so unknown words still spawn a rough parcel. Tool words (flute, broom, cloth…) spawn physical pushable objects — `E` picks them up, double-`E` sets them back down. `hook`/`grapple` throws and anchors, dropping a swingable rope — aimed with the mouse (dotted preview + landing ring), loosed with click or `Space`. |
| **Use key (`U`)** | `E` takes/interacts (wipe, take, grab, tie, examine); `U` uses the right held thing on the nearby problem — play an instrument, wave a broom, burn a herb. Handheld tools (cloth/music/broom) max 2; double-tap `E` sets the oldest down. Herbs are counted with per-plant yields; uses consume one. |
...
`behavior` is one of `static` (platform), `float` (pauses ~2s for boarding,
then rises, carries rider),
